...
object InvoiceGenerator {
    ...
    fun generateDayCloseContent(data: DayCloseData): String {
        ...
        builder.appendLine("Monto total        : RD$ ${"%,.2f".format(data.totalPayments)}")
        builder.appendLine("Préstamos otorgados: RD$ ${"%,.2f".format(data.totalLoans)}")
        builder.appendLine("Transacciones      : ${data.transactionsCount}")
        builder.appendLine("Saldo inicial      : RD$ ${"%,.2f".format(data.initialBalance)}")
        builder.appendLine("Saldo final        : RD$ ${"%,.2f".format(data.finalBalance)}")
        builder.appendLine("-".repeat(32))
        ...
        if (data.payments.isNotEmpty()) {
            ...
            data.payments.forEachIndexed { index, fee ->
                ...
                builder.appendLine(
                    "Cuota: ${fee.number}/${fee.numberTotal}  Monto: RD$ ${
                        "%,.2f".format(
                            fee.paymentAmount
                        )
                    }"
